...
6. **Samples**: large data (photo libraries, mail, home directories) is served straight from the backup through a read-only overlay, so the restored applications see all of it while only what is read is fetched. The newest N objects of each root are then read through that overlay for evidence (file names, EXIF dates, e-mail subjects, commit info...). Where a restore server cannot mount, the same objects are extracted instead (`Sampling` -> `How the restore server gets large data`).
