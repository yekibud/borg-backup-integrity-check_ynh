...
6. **Samples**: the newest N objects of each large root are extracted directly into place (sampled mode) or the whole root is extracted (full mode). The small files an app's own restore script needs (scripts and configuration inside `backups/`, `conf/`, `db/`... directories) are restored even when the directory around them is too big to keep. Evidence (file names, EXIF dates, e-mail subjects, commit info...) is extracted generically from the objects themselves.
7. **Verify**: services, HTTP endpoints, databases; sampled objects are looked up in the application database and, for mail, through Dovecot. Each sampled object is also checked against its own name: a file called `.jpg` whose bytes are not an image - and do not compress - is reported as suspicious, which is what data encrypted in place by ransomware looks like in an otherwise valid backup.
